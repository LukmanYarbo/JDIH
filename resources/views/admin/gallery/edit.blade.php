...
@section('title', 'Edit Galeri - JDIH DPRD Bolmut')
            ...
            <div class="card border-0 shadow-sm p-4 bg-white">
                <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul" id="judul" class="form-control @error('judul') is-invalid @enderror" placeholder="Ketikkan judul galeri..." value="{{ old('judul', $gallery->judul) }}" required>
                        @error('judul')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="tipe" id="tipeFoto" value="foto" {{ old('tipe', $gallery->tipe) == 'foto' ? 'checked' : '' }}>
                            <label class="form-check-label fs-7" for="tipeFoto">
                                <i class="bi bi-camera me-1"></i> Foto / Gambar
                            </label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="tipe" id="tipeVideo" value="video" {{ old('tipe', $gallery->tipe) == 'video' ? 'checked' : '' }}>
                    ...
                    <div class="mb-3" id="photoGroup">
                        <label for="file_path" class="form-label fs-7 fw-semibold text-muted">Berkas Foto</label>
                        @if($gallery->file_path)
                            <div class="mb-2">
                                <img src="{{ asset($gallery->file_path) }}" alt="{{ $gallery->judul }}" class="rounded border" style="max-height: 150px; object-fit: cover;">
                        ...
                        <small class="text-muted fs-8">Biarkan kosong jika tidak ingin mengubah foto. Max: 5MB. Format: jpeg, png, jpg, webp</small>
                        @error('file_path')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 d-none" id="videoGroup">
                        ...
                        <input type="url" name="video_url" id="video_url" class="form-control @error('video_url') is-invalid @enderror" placeholder="Contoh: https://www.youtube.com/embed/dQw4w9WgXcQ" value="{{ old('video_url', $gallery->video_url) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="4" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Tuliskan keterangan mengenai foto atau video ini...">{{ old('keterangan', $gallery->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
