...
<aside id="adminSidebar">
    <div class="sidebar-header">
        <a href="{{ route('portal.home') }}" class="sidebar-brand">
            @if(isset($gProfil) && $gProfil->logo)
                <img src="{{ asset($gProfil->logo) }}" alt="Logo DPRD" class="me-2" style="height: 32px; width: auto; object-fit: contain;">
            @else
                <i class="bi bi-bank2 text-warning fs-3 me-2"></i>
            @endif
            <div>
                <span class="fs-6 d-block lh-1 text-white">JDIH DPRD</span>
                <small class="fs-9 text-white-50 font-monospace">BOLAANG MONGONDOW UTARA</small>
            </div>
        </a>
    </div>
    <div class="sidebar-menu">
        ...
        @if(auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Operator'))
            ...
            <a href="{{ route('admin.documents.index') }}" class="sidebar-item {{ Route::is('admin.documents.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.news.index') }}" class="sidebar-item {{ Route::is('admin.news.*') ? 'active' : '' }}">
                <i class="bi bi-newspaper"></i> Berita Hukum
            </a>
        ...
        @if(auth()->user()->hasRole('Admin'))
            ...
            <a href="{{ route('admin.users.index') }}" class="sidebar-item {{ Route::is('admin.users.*') ? 'active' : '' }}">
    ...
    <div class="sidebar-footer">
        <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center gap-2">
                <div class="bg-warning rounded-circle d-flex align-items-center justify-content-center text-dark" style="width: 32px; height: 32px; font-weight: bold;">
                    {{ substr(auth()->user()->name, 0, 1) }}
                </div>
                <div>
                    <span class="fs-7 d-block text-white fw-semibold" style="max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ auth()->user()->name }}</span>
